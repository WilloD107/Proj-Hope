...
"Victory. Such a tempting thing. Naturally, everyone wants to savor a taste." - Hope von Degurechaff
...
We'll ensure that non-root users are unable to see the `project-Hope` service.
...
We'll ensure that `project-Hope` can be compiled with *.NET*.
...
We'll build `project-Hope`, so we can register it as a service:
